...
export class MailService {
  ...
  async sendVerificationEmail(email: string, token: string): Promise<void> {
    ...
    const fullVerifyUrl =
      ...
      `${this.configService.get('FRONTEND_URL', 'http://localhost:40000/api/auth')}/verify-email?token=${token}`;

    // Create a shortened URL for better email formatting
    const shortUrlData = await this.urlShortenerService.createShortUrl({
      originalUrl: fullVerifyUrl,
      expiresInHours: 24, // Match the email verification token expiry
    });
    
    const verifyUrl = shortUrlData.shortUrl;
    this.logger.log(`✅ Created shortened URL for email verification: ${shortUrlData.shortCode}`);
  ...
  async sendPasswordResetEmail(email: string, token: string): Promise<void> {
    // Create the full password reset URL
    const fullResetUrl =
      this.urlConfigService.getPasswordResetUrl(token) ||
      `${this.configService.get('FRONTEND_URL', 'http://localhost:40000/api/auth')}/reset-password?token=${token}`;

    // Create a shortened URL for better email formatting
    const shortUrlData = await this.urlShortenerService.createShortUrl({
      originalUrl: fullResetUrl,
      expiresInHours: 1, // Password reset tokens expire in 1 hour
    });
    
    const resetUrl = shortUrlData.shortUrl;
    this.logger.log(`✅ Created shortened URL for password reset: ${shortUrlData.shortCode}`);
    ...
    const html = `
      ...
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.configService.get('FRONTEND_URL', 'http://localhost:40000/api/auth')}/account/security"
               style="background: #dc3545; color: white; padding: 15px 30px; text-decoration: none;
                      border-radius: 5px; font-weight: bold; display: inline-block; font-size: 16px;">
    ...
    const html = `
      ...
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.configService.get('FRONTEND_URL', 'http://localhost:40000/api/auth')}/account/sessions"
