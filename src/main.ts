...
async function bootstrap() {
  ...
  // Apply CSRF middleware (exclude URL shortener endpoints that use Bearer tokens)
  ...
  app.setGlobalPrefix('api', {
    exclude: [
      ...
      // URL Shortener endpoints (exclude from /api prefix)
      { path: 's/:shortCode', method: RequestMethod.GET },
      { path: 'url-shortener/*path', method: RequestMethod.ALL },
    ],
  });
  ...
  const port = configService.get('port', 40000);
